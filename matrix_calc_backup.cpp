...
int node_Max(std::vector<std::string> nodeVector, int& node_max) {
...
void mat_Populate(std::vector<std::vector<std::string>> const& nodeVectors, const int& node_max, float& omega, Eigen::MatrixXcd& Cond, Eigen::VectorXcd& Volt, Eigen::VectorXcd& Curr) {
    ...
    for (int i = 0; i < nodeVectors.size(); i++) {
        type = nodeVectors[i][0]; // First string in a line of strings indicates the type of component
    ...
    for (int j = 0; j < nodeVectors.size(); j++) {

        type = nodeVectors[j][0];
        ith = std::stoi(nodeVectors[j][1]) - 1; // ith position ( "-1" is there because Eigen::Matrix matrices start from [0,0]
        jth = std::stoi(nodeVectors[j][2]) - 1; // jth position --
        std::cout << ith << " " << jth << std::endl;
        val = std::stod(nodeVectors[j][3]);
...
int main(){
    ...
    float omega = 1000; // Omega from netlist
    int node_max = 0;

    std::vector<std::vector<std::string>> v{
        {"C", "1", "2", "0.1"},
        {"C", "2", "0", "0.1"},
        {"VS", "1", "0", "1"}// Netlist in the format { "TYPE" , "NODE A" , "NODE B" , "VALUE" }
    };

    for (int i = 0; i < v.size(); i++) {
        node_max = node_Max(v[i], node_max);
    }
    ...
    mat_Populate(v, node_max, omega, cond_Mat, volt_Mat, curr_Mat);
    ...
    std::cout << cond_Mat.partialPivLu().solve(curr_Mat) << std::endl;
